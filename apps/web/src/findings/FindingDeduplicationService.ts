/**
 * FindingDeduplicationService – merges rule and AI findings into canonical findings.
 * Uses normalized type + overlapping element IDs for deterministic matching.
 */

import type { NormalizedFinding } from "./canonicalTypes";
import type { CanonicalFinding, FindingSource } from "./canonicalTypes";
...
export function deduplicateFindings(normalized: NormalizedFinding[]): CanonicalFinding[] {
  ...
  for (const rule of ruleFindings) {
    ...
    const primarySource: FindingSource = matchingAi.length > 0 ? "RULE_BASED" : "RULE_BASED";
